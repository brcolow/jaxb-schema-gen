...
final String bindingsFilePath = project.properties['jaxb.bindings']
final String xsdFilePath = project.properties['jaxb.xsd']
...
if (runXjc) {
    ...
    processBuilder.command(System.getenv("JAVA_HOME") + "/bin/java", "-classpath",
            ...
            '-b', bindingsFilePath,
            '-p', project.properties['jaxb.pkg.prefix'] + "." + xsdFilePath.substring(xsdFilePath.lastIndexOf('/') + 1, xsdFilePath.lastIndexOf(('.'))),
            xsdFilePath)
...
String wsdlFilePaths = project.properties['jaxb.wsdl']
String[] wsdlFilePathArr = wsdlFilePaths.indexOf(';') != -1 ? wsdlFilePaths.split(';') : [wsdlFilePaths]
for (String wsdlFilePath : wsdlFilePathArr) {
    System.out.println("WSDL File: " + wsdlFilePath)
    ...
    if (cacheDirExists) {
        if (wsdlChecksumFile.exists()) {
            ...
            if (cachedWsdlChecksum == latestWsdlChecksum) {
                runWsimport = false
                System.out.println("Skipping wsimport because WSDL file ${wsdlFilePath.substring(wsdlFilePath.lastIndexOf('/'), wsdlFilePath.length())} hasn't changed")
            }
        } else {
            runWsimport = true
        }
    ...
    if (runWsimport) {
        System.out.println("Running wsimport on WSDL: ${wsdlFilePath.substring(wsdlFilePath.lastIndexOf('/'), wsdlFilePath.length())}")
        ...
        String[] wsImportArgs = ['-verbose', '-keep',
                                 ...
                                 '-extension', '-Xtostring',
                                 '-p', project.properties['jaxb.pkg.prefix'] + "." + wsdlFilePath.substring(wsdlFilePath.lastIndexOf('/') + 1, wsdlFilePath.lastIndexOf(('.'))),
                                 wsdlFilePath]
